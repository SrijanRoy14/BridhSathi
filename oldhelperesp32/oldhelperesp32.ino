	...
	switch(type) {
      ...
      image_buffer=(uint8_t*)malloc(96*96);
            if (length == 96*96) {
                ...
                if(predicted_class>1) webSocket.sendTXT(predicted_class);
                else webSocket.sendTXT("Something went wrong with the inferencing");
...
void setup() {
  ...
  //Serial.setDebugOutput(true);
	...
	webSocket.begin("192.168.0.102", 8080, "/ws/testing/");
...
int run_inference(uint8_t* image_buffer,size_t length)
{
  ...
  if(image_buffer)
  {
    ...
    if(!fmt2rgb888(image_buffer, length, ei_camera_capture_out)){ Serial.println("Camera => Cannot convert the Grayscale buffer to RGB888!"); return -1; }
    ...
    for(size_t ix = 0; ix < EI_CLASSIFIER_OBJECT_DETECTION_COUNT; ix++){
      ...
      if(bb.label == "fine") return 0;
      if(bb.label == "danger") return 1;
      
      if(bb.label == "stolen") return 2;
      if(bb.label == "call") return 3;
